...
var recoverVideo = function(videoId) {

    var flag = window.confirm("是否恢复视频");
    if (!flag) {
        return;
    }

    $.ajax({
        url: $("#hdnContextPath").val() + "/video/recoverVideo.action?videoId=" + videoId,
        type: "POST",
        async: false,
        success: function(data) {
            if(data.status == 200 && data.msg == "OK") {
	...
	var handleUsersReportsList = function() {
        ...
        jqGrid.jqGrid({  
            ...
            colModel: [  
                ...
                { name: 'ts', index: 'ts', width: 40, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
                ...
                { name: '', index: '', width: 20, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
                        if(rowObject.status == 1){
                            ...
                            var button = '<button class="btn btn-outline blue-chambray" id="" onclick=recoverVideo("' + rowObject.dealVideoId + '") style="padding: 1px 3px 1px 3px;">恢复播放</button>';
                        }
			    		return button;
			    	}
			    }
